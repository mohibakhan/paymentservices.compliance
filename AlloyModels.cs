...
public sealed class AlloyTmsResponse
{
    ...
    /// <summary>
    /// Outcome resolved from evaluations[0].summary.outcome.
    /// Falls back to Approved if evaluations is empty (should not happen in practice).
    /// </summary>
...
public sealed class AlloyTmsEventResult
{
    ...
    public List<AlloyTmsEvaluation> Evaluations { get; set; } = [];

    [JsonPropertyName("journey_applications")]
    public List<object> JourneyApplications { get; set; } = [];
}

public sealed class AlloyTmsEvaluation
{
    [JsonPropertyName("evaluation_token")]
    public string? EvaluationToken { get; set; }

    [JsonPropertyName("workflow_token")]
    public string? WorkflowToken { get; set; }

    [JsonPropertyName("prefiltered")]
